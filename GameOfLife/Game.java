...
public class Game {
  ...
  public void newGame() {
    ...
    // We also set the neighbours of the objects since this is only runed the first time
    setNeighbours();
  }
  ...
  public void newGeneration() {
    ...
    for (int i = 0; i < board.size(); i++) {
      for (int j = 0; j < board.get(0).size(); j++) {
        board.get(i).get(j).proposeChanges();
      }
    }
    // make changes
    for (int i = 0; i < board.size(); i++) {
      for (int j = 0; j < board.get(0).size(); j++) {
        board.get(i).get(j).makeChanges();
      }
    }
  }
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    System.out.println(
      "This is a 0 player game. It is composed of a 30X30 matirx of alive or dead cells."
    );
    System.out.println(
      "Each turn, rules will aplay to all cells (refer to Rules.png)."
    );
    System.out.println("And the result will be printed on the terminal.");
    System.out.println(
      "Alive cells will be represented as ■. And dead cells will be represented with ·\n"
    );
    ...
    if (response == 1) {
      System.out.println(
        "I'll ask for a X and Y coordinates. The cell in that positio will be alive."
      );
